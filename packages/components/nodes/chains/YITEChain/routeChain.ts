...
class RouteChain_Chains implements INode {
    label: string
    name: string
    version: number
    type: string
    ...
    constructor(fields?: { sessionId?: string }) {
        this.label = 'Route Chain'
        this.name = 'routeChain'
        this.version = 1.0
        this.type = 'ConversationalRetrievalQAChain'
        this.icon = 'chain.svg'
        this.category = 'Chains'
        this.description = 'Chain to detect the user intent for routing the conversation'
        ...
        this.outputs = [
            {
                label: 'Route Chain',
                name: 'routeChain',
                baseClasses: [this.type, ...getBaseClasses(ConversationalRetrievalQAChain)]
            },
            {
    ...
    async run(nodeData: INodeData, input: string, options: ICommonObject): Promise<string | object> {
        ...
        console.log("Session ID routeChain: ", this.sessionId)
        const historyMessages = await memory.getChatMessages(this.sessionId, true)
...
module.exports = { nodeClass: RouteChain_Chains }
